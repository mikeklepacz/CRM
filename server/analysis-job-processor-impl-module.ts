...
import { buildAnalysisVariables, buildWickCoachSection } from './analysis-job-prompt-utils';
...
async function processJob(jobId: string) {
  try {
    ...
    for (let i = 0; i < callsData.length; i += CALLS_PER_BATCH) {
      ...
      const variables = buildAnalysisVariables(transcriptContext, kbContext, insight, wickCoachSection);
