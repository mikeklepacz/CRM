...
import { findStoreSheetRowByLink, resolveStoreDatabaseSheet } from "../../../services/sheets/storeDatabaseResolver";
...
export function buildCreateParentDbaHandler(deps: DbaRouteDeps) {
  ...
  return async (req: any, res: any) => {
    try {
      ...
      const trackerCellRange = (colLetter: string, row: number) => buildSheetRange(trackerSheet.sheetName, `${colLetter}${row}`);
      // Read tracker data
      ...
      if (parentLink) {
          ...
          if (foundRowIndex !== -1) {
              ...
              if (isParentIndex !== -1) {
                  ...
                  updates.push({
                      range: trackerCellRange(colLetter, foundRowIndex),
                      values: [['TRUE']]
                  });
              }
              if (dbaIndex !== -1) {
              ...
              if (notesIndex !== -1) {
                              ...
                              if (normalizeLink(trackerRows[i][linkIndex] || '') === normalizedChild) {
                                  ...
                                  if (!finalPocName && pocNameIndex !== -1) {
                                      ...
                                      if (val) {
                                          ...
                                          updates.push({ range: trackerCellRange(col, foundRowIndex), values: [[val]] });
                                      }
                                  }
                                  if (!finalPocEmail && pocEmailIndex !== -1) {
                  ...
                  if (aggregatedNotes !== existingNotes) {
                      ...
                      const existingUpdate = updates.findIndex(u => u.range === trackerCellRange(colLetter, foundRowIndex));
                      if (existingUpdate !== -1) {
      ...
      let storeSheet: any = null;
      if (childLinks && Array.isArray(childLinks) && childLinks.length > 0) {
          const firstChildMatch = await findStoreSheetRowByLink({
              tenantId: (req.user as any).tenantId,
              link: childLinks[0],
              preferProjectMatch: true,
          });
          storeSheet = firstChildMatch?.sheet || null;
      }
      if (!storeSheet) {
          storeSheet = await resolveStoreDatabaseSheet({
              tenantId: (req.user as any).tenantId,
              preferProjectMatch: true,
          });
      }
      ...
      await googleSheets.appendSheetData(storeSheet.spreadsheetId, buildSheetRange(storeSheet.sheetName, "A:ZZ"), [storeRow]);
      ...
      await googleSheets.appendSheetData(trackerSheet.spreadsheetId, buildSheetRange(trackerSheet.sheetName, "A:ZZ"), [trackerRow]);
      clearUserCache(userId);
      res.json({
          success: true,
