...
export function buildOrderMatchEndpointHandler(deps: OrderMatchRouteDeps) {
    ...
    try {
      ...
      for (const store of storeLinks) {
          ...
          if (existingTrackerRowIndex > 0) {
              ...
              // Set Link
              if (linkIndex !== -1)
                  newRow[linkIndex] = storeLink;
              // Set Order ID
              if (orderIdIndex !== -1)
                  newRow[orderIdIndex] = order.orderNumber;
              // Set Transaction ID
              if (transactionIdIndex !== -1)
                  newRow[transactionIdIndex] = order.id;
              // Set Agent Name
              if (agentNameIndex !== -1 && agentName)
      ...
      for (const store of storeLinks) {
          ...
          const existingClient = await db.query.clients.findFirst({
              where: and(eq(clients.uniqueIdentifier, normalizedLink), eq(clients.tenantId, tenantId)),
          });
          if (existingClient) {
      ...
      if (storeLinks.length > 0) {
          ...
          const primaryClient = await db.query.clients.findFirst({
              where: and(eq(clients.uniqueIdentifier, primaryStoreLink), eq(clients.tenantId, tenantId)),
          });
          if (primaryClient) {
