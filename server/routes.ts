...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Helper function to check if user has admin access (used for inline checks)
  // This fetches roleInTenant from user_tenants table since it's not on the user object
  const checkAdminAccess = async (user: any, tenantId: string | undefined): Promise<boolean> => {
    ...
    if (tenantId) {
      ...
      if (roleInTenant === 'org_admin') return true;
    }
    return false;
  };
  ...
  // Stream/download background audio file
  app.get('/api/voice-proxy/background-audio/file', isAuthenticatedCustom, isAdmin, async (req: any, res) => {
    try {
      const settings = await storage.getBackgroundAudioSettings();
...
- Output ONLY the JSON, no additional commentary`;
  ...
  // Delete a single proposal (admin only)
...
Use this store information to provide context-aware responses. When helping draft emails or communications, reference specific details about this store.`;
  ...
  // Get email send failures (admin only)
  app.get('/api/ehub/email-failures', isAuthenticatedCustom, isAdmin, async (req: any, res) => {
    try {
      const limit = Math.min(parseInt(req.query.limit as string) || 50, 500);
      const offset = parseInt(req.query.offset as string) || 0;

      // Get recipients with 'failed' status and their last message attempt
...
${conversationContext}`;
  ...
  // Get sequence steps (admin only)
  app.get('/api/sequences/:id/steps', isAuthenticatedCustom, isAdmin, async (req: any, res) => {
    try {
      const { id } = req.params;
  ...
  // Update individual sequence step (admin only)
  app.patch('/api/sequences/:sequenceId/steps/:stepId', isAuthenticatedCustom, isAdmin, async (req: any, res) => {
    try {
  ...
  app.get('/api/sequences/:id/recipients', isAuthenticatedCustom, isAdmin, async (req: any, res) => {
    try {
      ...
      const enrichedRecipients = recipients.map(recipient => {
        ...
        return { 
          ...recipient,
          // Override name, link, and salesSummary with fresh tracker data if available
