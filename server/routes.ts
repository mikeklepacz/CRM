...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/banned-words', isAuthenticatedCustom, async (req: any, res) => {
    try {
      const userId = req.user.isPasswordAuth ? req.user.id : req.user.claims.sub;
      const user = await storage.getUser(userId);
      ...
      const bannedWords = await storage.getAllBannedWords();
      res.json(bannedWords);
    } catch (error: any) {
      console.error("Error fetching banned words:", error);
      res.status(500).json({ message: error.message || "Failed to fetch banned words" });
    }
  });

  // Add banned words
  app.post('/api/banned-words', isAuthenticatedCustom, async (req: any, res) => {
    try {
      const userId = req.user.isPasswordAuth ? req.user.id : req.user.claims.sub;
      const user = await storage.getUser(userId);
      ...
      const { words, type } = req.body; // words is an array of strings, type is 'keyword' or 'tag'
      
      if (!words || !Array.isArray(words) || words.length === 0 || !type) {
        return res.status(400).json({ message: "Words array and type are required" });
      }

      if (type !== 'keyword' && type !== 'tag') {
        return res.status(400).json({ message: "Type must be 'keyword' or 'tag'" });
      }

      const added = await storage.addBannedWords(words, type, userId);
      res.json({ message: "Banned words added successfully", added });
    } catch (error: any) {
      console.error("Error adding banned words:", error);
      res.status(500).json({ message: error.message || "Failed to add banned words" });
    }
  });

  // Delete a banned word
  app.delete('/api/banned-words/:id', isAuthenticatedCustom, async (req: any, res) => {
    try {
      const userId = req.user.isPasswordAuth ? req.user.id : req.user.claims.sub;
      const user = await storage.getUser(userId);
      ...
      const { id } = req.params;
      await storage.deleteBannedWord(id);
      res.json({ message: "Banned word removed successfully" });
    } catch (error: any) {
      console.error("Error deleting banned word:", error);
      res.status(500).json({ message: error.message || "Failed to delete banned word" });
    }
  });
