...
export const userPreferences = pgTable("user_preferences", {
  ...
  defaultMapCountry: varchar("default_map_country", { length: 100 }), // Default country for Map Search
  defaultMapView: jsonb("default_map_view").$type<{ lat: number; lng: number; zoom: number }>(), // Default map center and zoom for Map Search
  ...
  visibleModules: jsonb("visible_modules").$type<Record<string, boolean>>().default(sql`'{}'::jsonb`), // Which navigation modules are visible
...
// Each email account in the pool gets its own set of slots (dailyLimit per account)
export const dailySendSlots = pgTable("daily_send_slots", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").notNull().references(() => tenants.id),
  emailAccountId: varchar("email_account_id").references(() => emailAccounts.id, { onDelete: 'cascade' }), // Which email account this slot belongs to
