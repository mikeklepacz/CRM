...
- **E-Hub Repeat Last Step**: Optional feature allows the final step to repeat indefinitely until reply is detected. When enabled (repeatLastStep=true), after sending the last step, the system schedules another send using the last gap delay (stepDelays[length-1]) instead of marking the sequence as completed. Repeat logic uses >= comparison to handle currentStep beyond stepDelays.length.
- **E-Hub Reply Detection**: Two-gate pre-send system checks Gmail for replies at two specific times: (1) scheduledAt - 60 seconds (pre-check), (2) scheduledAt (final check). Centralized in `replyGuard.ts` service. No background polling. When reply detected: marks scheduled send as cancelled, deletes future sends, updates recipient status to 'replied', clears nextSendAt. Dramatically reduces Gmail API quota usage.
- **E-Hub AI Email Generation**: Uses OpenAI Assistant `asst_H3RlcWO2lTuGnFvHI47elI8d` (specialized email writing assistant with knowledge base) via Assistants API. Campaign Brief (finalizedStrategy) is MANDATORY - no fallbacks. Assistant receives Campaign Brief + recipient context, leverages knowledge base for email best practices, deliverability principles, and tone guidance. Generates subject + HTML body with proper formatting, hyperlinks, and signature. All email operations use this assistant - never vanilla ChatGPT.
- **E-Hub Queue Coordinator**: Centralized scheduling system enforcing FIFO ordering, rate limiting, and geographic distribution. All recipient scheduling operations (bulk-add, individual add, step progression) funnel through Queue Coordinator for consistent slot allocation. Uses mutex lock to serialize bulk operations and prevent concurrent race conditions. Implements cohort-based timezone balancing: groups recipients by rounded minute, applies round-robin timezone interleaving within each cohort, preserves chronological ordering across cohorts. Rate limiting uses continuous spacing (minutesBetweenSends = adminWindowMinutes / dailyRateLimit) to evenly distribute sends across admin window. Two-tier priority: follow-ups (currentStep > 0) processed before fresh emails (currentStep = 0). Queue tail tracking ensures new enrollments join end of queue regardless of step delay. Supports admin timezone auto-detection when traveling. Integration with smart timing service for dual-window validation (admin + client business hours) and weekend skipping.
- **Matrix2 Slot-First Scheduler (Nov 2025)**: Production email scheduling system replacing all legacy schedulers (matrixScheduler, emailSchedulingService, smartTiming quarantined as of Nov 17 2025). Uses slot-first architecture: pre-generates daily email slots with jittered timing based on `ehub_settings` (sending_hours_start/end, daily_email_limit, min/max_delay_minutes), assigns eligible recipients to open slots, processes sends from `daily_send_slots` table. Honors admin timezone from `user_preferences`, enforces recipient business hours and step delays. Single source of truth: `ehub_settings` controls ALL timing decisions. Components: `slotGenerator.ts` (creates slots), `slotAssigner.ts` (matches recipients), `slotDb.ts`/`recipientDb.ts` (database operations), `emailQueue.ts` (sends from slots). Migration complete Nov 17 2025 - all enrollment flows updated to ONLY use Matrix2, old schedulers replaced with error-throwing stubs.
