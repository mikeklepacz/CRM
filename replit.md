...
- **Technical Implementations**:
    ...
    - **WooCommerce Sync**: Fetches orders, matches them to stores, calculates commissions, and updates the Commission Tracker. After syncing, automatically backfills any missing commission records for all historical orders in the database.
    - **Referral Commission System**: Single-level referral tracking where referring agents earn 10% of their referred agents' monthly commissions (additive - company pays both commissions). Uses ledger approach with separate commission records (`commissionKind='referral'`). Supports all commission types including one-time and recurring. Referral earnings displayed in Sales Analytics dashboard showing individual referred agents with breakdown of earnings per agent.
    - **Inline Commission Editing**: Admins can edit commission type and amount directly in the WooCommerce sync table. Changes auto-save after 1 second debounce and are flushed immediately when clicking "Sync Orders" to ensure accurate referral calculations.
