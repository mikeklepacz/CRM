...
- **E-Hub Reply Detection**: Background worker runs every 5 minutes checking all active threads (status='in_sequence', threadId exists, repliedAt=null) using Gmail API. When reply detected: sets repliedAt timestamp, updates replyCount, changes status to 'replied', clears nextSendAt to stop future sends, and updates sequence repliedCount stats. Queue filtering excludes recipients with repliedAt set.
