...
- **Matrix Scheduler (Nov 2025)**: Mission-critical scheduling system enforcing simultaneous global (admin window, rate limits) and recipient-specific (business hours, timezone, legal delivery) constraints. Replaced deprecated Smart Timing. 14-day search window finds valid overlaps between admin sending hours and recipient business hours across timezones. **CRITICAL FIX**: Uses `formatInTimeZone(date, 'UTC', 'yyyy-MM-dd')` for base date extraction to prevent timezone interpretation mismatches. The `format()` function from date-fns uses server's local timezone (not UTC), causing admin and recipient windows to reference different calendar days. UTC-based extraction ensures both timezones build from identical reference point.
